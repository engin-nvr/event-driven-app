...
resource "aws_instance" "master" {
  ...
  # Dosya kopyaama
  provisioner "file" {
    source      = "./mykey.pem" # Yerel dosyanın yolu
    destination = "/home/ubuntu/.ssh/mykey.pem"                            # Uzak hedef dosya yolu
    connection {
      type        = "ssh"
      user        = "ubuntu"
      private_key = file("./mykey.pem") # Anahtar dosyasının yolu
      host        = self.public_ip
    }
  }
}
...
# IAM rolüne bağlı politika tanımlaması
